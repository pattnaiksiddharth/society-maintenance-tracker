...
export const ComplaintStatsChart: React.FC<ComplaintStatsChartProps> = ({
  ...
  const strokeOffset = circumference - completedStroke;

  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 shadow-2xl shadow-black/20 flex flex-col justify-between">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="px-2.5 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-mono font-medium">
          {completedPercent}% Closed
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-around gap-6 py-2">
          ...
          <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 120 120">
            ...
            {totalCount > 0 && (
              <circle
                ...
                cy="60"
                r={radius}
                className="text-amber-500/80 transition-all duration-1000 ease-out"
                strokeWidth="14"
        ...
        <div className="flex flex-col gap-3.5 w-full sm:w-auto">
          ...
          <div className="flex items-center justify-between sm:justify-start gap-4 p-3 rounded-2xl bg-white/5 border border-white/5">
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 px-1 pt-1 border-t border-white/5">
            ...
            <span className="font-semibold text-emerald-400">{completedPercent}%</span>
          </div>
        </div>
      </div>
    </div>
